...
for file in ./store-implementation/accumulo-store/src/test/resources/*.properties; do
	sed -i "s|^gaffer.store.class=.*Single.*$|gaffer.store.class=uk.gov.gchq.gaffer.accumulostore.SingleUseAccumuloStore|" $file
	sed -i "s|^gaffer.store.class=.*$|gaffer.store.class=uk.gov.gchq.gaffer.accumulostore.AccumuloStore|" $file
