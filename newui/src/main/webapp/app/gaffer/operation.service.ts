...
export class OperationService {
    ...
    getAvailableOperations = function() {
        if (this.availableOperations) {
            return of(this.availableOperations);
        } else if (!this.availableOperationsObservable) {
            this.availableOperationsObservable = Observable.create((observer: Observer<String>) => {
                this.reloadOperations(false,observer);
            });
        }

        return this.availableOperationsObservable;
    }

    private hasInputB = function(first, availableOps) {
        for (var i in availableOps) {
            if (availableOps[i].class && this.common.endsWith(availableOps[i].class, first)) {
    ...
    private addOperations = function(operations, conf) {
        if(operations) {
            for (var i in operations) {
                ...
                if(this.opAllowed(op.name, conf.operations) && this.canHandleOperation(op)) {
                    var fields = {};
                        ...
                        if(op.parameters) {
                            for(var j in op.parameters) {
                                ...
                                if(op.parameters[j].defaultValue) {
                                    ...
                                    op.parameters[j].parts = this.types.createParts(valueClass, op.parameters[j].defaultValue);
                                } else {
    ...
    reloadOperations = function(loud, observer) {

        this.config.get().then(function(conf) {
            var queryUrl = this.common.parseUrl(conf.restEndpoint + "/graph/operations/details");
            this.http.get(queryUrl)
                .then(function(response){
                    ...
                    if(this.common.arrayContainsObjectWithValue(this.availableOperations, "class", getAllClass)) {
                        this.query.execute(
                            {
                                class: getAllClass,
                            ...
                            function(err) {
                                if (loud) {
                                    this.error.handle('Failed to load named operations', null, err);
                                    observer.error(err);
                                }
                                observer.next(this.availableOperations);
                            }
                ...
                function(err) {
                    this.error.handle('Unable to load operations', null, err.data);
                    observer.next([]);
                });
        });
    }
    ...
    ifOperationSupported = function(operationClass, onSupported, onUnsupported) {
        this.config.get().then(function(conf) {
            var queryUrl = this.common.parseUrl(conf.restEndpoint + "/graph/operations");
            ...
            this.http.get(queryUrl)
                ...
                function(err) {
                    this.error.handle('Error getting available graph operations', err.data);
                    onUnsupported();
            });
        })
    }
