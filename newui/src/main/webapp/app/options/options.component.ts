...
export class OptionsComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if (!this.model) {    // If the model is not yet defined, it must get the default from somewhere.
    ...
    private saveToService = function() {
        if (this.master) {        // If master is being destroyed, for example when the user navigates away, the service is updated
    ...
    clearValue = function(index) {
        ...
        this.model.visible[index].value = Array.isArray(currentValue) ? [] : undefined;
    }
    ...
    addOption = function() {
        ...
        this.model.visible.push(cloneDeep(this.selectedOption));
    ...
    getValues = function(option) {
        var searchTerm = this.searchTerms[option.key] ? this.searchTerms[option.key].toLowerCase() : this.searchTerms[option.key];
        ...
        if (option.autocomplete.options) {
            ...
            this.query.execute(operation, function(values) {
                if (!searchTerm || searchTerm == "") {
