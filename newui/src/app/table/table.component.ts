    ...
    function TableController(
      ...
      vm.createVisualisation = function(ev) {
        $mdDialog
          .show({
            ...
            templateUrl:
              "app/table/visualisation-dialog/visualisation-dialog.html",
            targetEvent: ev,
          ...
          if (vm.data.types[t] in resultsByType) {
            for (var g in vm.data.groups) {
              if (vm.data.groups[g] in resultsByType[vm.data.types[t]]) {
                vm.data.results = vm.data.results.concat(
                  resultsByType[vm.data.types[t]][vm.data.groups[g]]
                );
              }
            }
          }
      ...
      vm.selectedColumnsText = function() {
        if (
          vm.data.columns &&
          vm.data.allColumns &&
          vm.data.allColumns.length > vm.data.columns.length
        ) {
          return (
            "Choose columns (" +
            (vm.data.allColumns.length - vm.data.columns.length) +
      ...
      var processResults = function(resultsData) {
        ...
        vm.data.allColumns = common.concatUniqueValues(
          common.concatUniqueValues(ids, groupByProperties),
          properties
        );
                  ...
                  if (elementDef && elementDef.properties) {
                    if (elementDef.groupBy) {
                      for (var j in elementDef.groupBy) {
                        ...
                        var typeDef =
                          vm.schema.types[elementDef.properties[propName]];
                        if (
                          typeDef &&
                          typeDef.description &&
                          !(propName in vm.data.tooltips)
                        ) {
      ...
      var cacheValues = function() {
        ...
        if (
          vm.data.columns &&
          vm.data.allColumns &&
          vm.data.columns.length < vm.data.allColumns.length
        ) {
        ...
        if (
          vm.data.types &&
          vm.data.allTypes &&
          vm.data.types.length < vm.data.allTypes.length
        ) {
        ...
        if (
          vm.data.groups &&
          vm.data.allGroups &&
          vm.data.groups.length < vm.data.allGroups.length
        ) {
