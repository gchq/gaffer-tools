...
export class OperationOptionsService {
  ...
  setDefaultConfiguration = function(newDefaults) {
    this.defaultOperationOptionsConfiguration = this.angular.copy(newDefaults);
  };
  ...
  getDefaultConfiguration = function() {
    return this.angular.copy(this.defaultOperationOptionsConfiguration);
  };
  ...
  getDefaultOperationOptions = function() {
    return this.extractOperationOptions(
      this.defaultOperationOptionsConfiguration
    );
  };
  ...
  extractOperationOptions = function(operationOptionsConfiguration) {
    if (operationOptionsConfiguration === undefined) {
      // undefined configuration implies explicitly that no options were configured
      return undefined;
    }

    var options = {};
    if (operationOptionsConfiguration === null) {
      // null configuration implies that the configuration has not been fetched yet
      return options;
    }
