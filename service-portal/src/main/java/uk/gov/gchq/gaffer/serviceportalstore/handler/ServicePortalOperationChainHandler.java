...
public class ServicePortalOperationChainHandler<OUT> extends OperationChainHandler<OUT> {

    public static final String SERVICE_PORTAL_STORE_OPERATION_CHAIN_HANDLER = "serviceportal.operationChainHandler";
    ...
    public ServicePortalOperationChainHandler(final OperationChainValidator opChainValidator, final List<OperationChainOptimiser> opChainOptimisers) {
    ...
    public OUT doOperation(final OperationChain<OUT> operationChain, final Context context, final Store store) throws OperationException {
        try {
            if (store instanceof ProxyStore) {
                ...
                throw new ServicePortalStoreException("Store was not an instance of ProxyStore, found: " + store);
            } catch (final ServicePortalStoreException | GafferWrappedErrorRuntimeException e) {
            }
            throw e;
        } catch (final Exception e) {
            throw new ServicePortalStoreException("Error occurred processing " + this.getClass(), e);
        }
    }

    private String getServicePortalOption(final OperationChain<OUT> operationChain) {
    ...
    private OUT unprocessedLogic(final OperationChain<OUT> operationChain, final Context context, final Store store) throws OperationException {
        ...
        rtn.addOption(SERVICE_PORTAL_STORE_OPERATION_CHAIN_HANDLER, RESOLVED);
    ...
    private OUT resolvedLogic(final OperationChain<OUT> operationChain, final Context context, final Store store) throws OperationException {
        ...
        for (final Operation operation : operationChain.getOperations()) {
            if (operation instanceof OperationChain) {
                ...
                if (TO_PROXY.equals(chain.getOptions().get(SERVICE_PORTAL_STORE_OPERATION_CHAIN_HANDLER))) {
                    return proxyLogic(operationChain, context, (ProxyStore) store);
                } else {
                    //Generic is of type Object
                    out = new ServicePortalOperationChainHandler<>(getOpChainValidator(), getOpChainOptimisers()).doOperation(chain, context, store);
                }
            } else {
                throw new ServicePortalStoreException("While resolving opChains for resolvedLogic, expected OperationChain found: " + operation);
            }
        }
        ...
        try {
            ...
            throw new ServicePortalStoreException("The return type was not of expected type", e);
        }
        return rtn;
    }
    ...
    private void ifAlternativeOperationsExists(final ArrayList<OperationChain> opChainOfOpChain, final ArrayList<Operation> operations, final boolean hasOperationHandler) {
        if (!operations.isEmpty()) {
            ...
            opchain.addOption(SERVICE_PORTAL_STORE_OPERATION_CHAIN_HANDLER, hasOperationHandler ? TO_HANDLERS : TO_PROXY);
            //Add op chain
