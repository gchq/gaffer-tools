...
angular.module('app').factory('table', ['common', 'types', 'time', 'events', function(common, types, time, events) {
    ...
    table.processResults = function(schema) {
        ...
        tableData.allColumns = common.dedupConcat(common.dedupConcat(ids, groupByProperties), properties);
        ...
        for(var type in tableData.resultsByType) {
            ...
            for(var group in tableData.resultsByType[type]) {
                common.dedupPush(group, tableData.allGroups);
            }
        }
    ...
    table.updateResultTypes = function() {
        ...
        for(var i in tableData.results) {
            common.dedupPushAll(Object.keys(tableData.results[i]), resultColumns);
        }
    ...
    var processElements = function(type, typePlural, idKeys, schema, ids, groupByProperties, properties) {
        if(results[typePlural] && Object.keys(results[typePlural]).length > 0) {
            ...
            common.dedupPushAll(idKeys, ids);
                    ...
                    if(element.properties) {
                        if(!(element.group in tableData.resultsByType[type])) {
                            ...
                            if(schema.entities[element.group] && schema.entities[element.group].groupBy) {
                                common.dedupPushAll(schema.entities[element.group].groupBy, groupByProperties);
                            }
                            if(schema[typePlural][element.group] && schema[typePlural][element.group].properties) {
                                 common.dedupPushAll(Object.keys(schema[typePlural][element.group].properties), properties);
                            }
                        }
                        for(var prop in element.properties) {
                            common.dedupPush(prop, properties);
            ...
            if(item) {
                ...
                for(var key in item) {
                    ...
                    if("class" === key) {
                        ...
                        common.dedupPush("type", ids);
                    } else if("vertex" === key) {
                        result["source"] = value;
                        common.dedupPush("source", ids);
                    } else if("source" === key) {
                        result["source"] = value;
                        common.dedupPush("source", ids);
                    } else if("value" === key) {
                        result[key] = value;
                        common.dedupPush(key, ids);
                    } else {
                        result[key] = value;
                        common.dedupPush(key, properties);
                    }
                }
